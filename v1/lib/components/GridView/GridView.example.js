...
import { GridViewType, FilterType, GridViewMessageType, FilterDataType, GridViewActionBarItems, QuickActionSectionAlignment, ControlType } from './GridView.models';
...
export const GridViewExample = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            data: 'number',
            onRender: (item) => {
                return (_jsx(HighlightText, { text: item.dateModified && item.dateModified.toDateString(), textToBeHighlighted: selectionsRef.current.searchText }, void 0));
            },
            isPadded: true,
            selected: true,
            searchable: true,
        ...
        },
        {
            ...
            data: 'number',
            selected: true,
            onRender: (item) => (_jsx("span", { children: item.isValid && item.isValid.toString() === 'true' ? 'Yes' : 'No' }, void 0)),
            ...
            onRenderEditControl: (item, onChange, column) => {
                return (_jsx(ChoiceGroup, { defaultSelectedKey: (item.isValid || item.isValid === false) && item.isValid.toString(), options: [
                        { key: 'true', text: 'Yes' },
                        { key: 'false', text: 'No' }
                    ], onChange: (ev, option) => {
    ...
    const onItemsUpdate = (items, operationType) => {
        console.log(items);
    };
    ...
    return (_jsxs(_Fragment, { children: [_jsx("button", Object.assign({ type: "button", onClick: onSetErrors }, { children: "Set Errors" }), void 0), _jsx("div", Object.assign({ className: "ms-Grid-row" }, { children: _jsx(GridView, { gridViewType: GridViewType.InMemory, columns: columns, items: items, totalRecords: items.length, pagingOptions: selections.pagingOptions, 
                    ...
                    // groupBy="column4"
                    searchPlaceHolderText: "Search By Date", itemUniqueField: "id", statusMessages: statusMessages, quickActionSectionItems: getQuickActionBarItems(), highLightSearchText: true, 
                    // allowSelection={true}
                    // allowGrouping={true}
