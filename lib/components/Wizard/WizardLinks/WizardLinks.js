...
export const WizardLinks = (props) => {
    ...
    const getWizardLinks = (wizardType, steps) => {
        ...
        return (wizardLinks &&
            wizardLinks.map((group) => (_jsx(Stack, Object.assign({ horizontal: wizardType === WizardType.Horizontal, className: classes.mainClass }, { children: !props.defaultStepLinksCollapse && wizardType === WizardType.Vertical && group.name ? (_jsx(Accordion, Object.assign({ headerText: group.name, accordionHeaderClass: classes.accordionHeader }, { children: group.links.map((link, index) => getWizardLink(wizardType, link, group.links, index)) }), void 0)) : (group.links.map((link, index) => getWizardLink(wizardType, link, group.links, index))) }), group.name))));
    };
    ...
    const getWizardLink = (wizardType, link, links, index) => {
        return (_jsxs(_Fragment, { children: [_jsxs(Stack, Object.assign({ "aria-label": `${link.name}: ${WIZARD_STEP_STATUS_STRINGS[getLinkStatus(link)]}`, title: `${link.name}: ${WIZARD_STEP_STATUS_STRINGS[getLinkStatus(link)]}`, onClick: () => {
                        ...
                        : mergeClassNames([
                            ...
                            props.defaultStepLinksCollapse ? classes.collapseLink : '',
                            props.stepClass
                        ]) }, { children: [_jsxs("button", Object.assign({ type: "button", disabled: link.disabled }, { children: [_jsx(Icon, { className: link.isCurrentItem ? 'active' : getClassName(link.status), iconName: link.icon
                                        ? link.isCurrentItem
                                            ? iconMaps[WizardStepStatus.Started]
                                            : link.icon
                                        : iconMaps[WizardStepStatus.Blocked] }, void 0), !props.defaultStepLinksCollapse && (_jsx("span", Object.assign({ className: link.isCurrentItem
                                        ? mergeClassNames([classes.activeStep, props.wizardLinkTextClass])
                                        : mergeClassNames([classes.stepText, props.wizardLinkTextClass]) }, { children: link.name }), void 0))] }), void 0), !props.defaultStepLinksCollapse &&
                            wizardType === WizardType.Vertical &&
                            index !== links.length - 1 && (_jsxs(_Fragment, { children: [_jsx("div", { className: links[index + 1].status === WizardStepStatus.Completed &&
                                        ...
                                        : link.status === WizardStepStatus.Completed
                                            ...
                                            : classes.connectorDashed }, void 0), getStepSummaryComponent(link.stepData)] }, void 0))] }), link.key), wizardType === WizardType.Horizontal && index !== links.length - 1 && (_jsx(Stack, Object.assign({ className: classes.horizontalSeparator }, { children: _jsx(Icon, { iconName: "ChevronRight" }, void 0) }), void 0))] }, void 0));
    };
